...
func TestTransportRejectsContentLengthSignWhenProcessingResponseHeaders(t *testing.T) {
	tests := []struct {
		name   string
		cl     []string
		wantCL string
	...
	for _, tt := range tests {
		...
		if got != tt.wantCL {
			t.Errorf("For case %q, value %q, got = %q; want %q", tt.name, tt.cl, got, tt.wantCL)
		}
	}
}
